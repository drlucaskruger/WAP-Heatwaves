...
lmp1<-lmPerm::lmp(recind~SICmean,data=cwsk,perm="Prob",center=T,singular.ok = F)

summary(lmp1)
anova(lmp1)

lmp2<-lmPerm::lmp(recind~TempMax,data=cwsk,perm="Prob",center=T,singular.ok = F)

summary(lmp2)
anova(lmp2)

lmp3<-lmPerm::lmp(recind~Wtemp,data=cwsk,perm="Prob",center=T,singular.ok = F)
...
# recruitment was better explained by winter sea ice
...
lmp4<-lmPerm::lmp(recind~IceCases,data=eveK2,perm="Prob",center=T,singular.ok = F, nCycle=1,maxIter=3000)
...
lmp5<-lmPerm::lmp(recind~TempCases,data=eveK2,perm="Prob",center=T,singular.ok = F, nCycle=1,maxIter=3000)
...
ggplot(eveK2,aes(IceCases,recind))+geom_boxplot()+
  ...
  theme_bw()+xlab("Events")+
  ylab("Recruitment index")+
  ggtitle(label="c. Low sea ice cover")+
  ...
  theme_bw()+xlab("Events")+
  ylab("Recruitment index")+
  ggtitle(label="d. High sea sea surface temperatures")


```


### conclusion: events of low winter sea ice cover explains better the year-round recruitment...

#### now that we have this information, let's see whether chlorophyll relates to sea ice 
#### summer clorophyll is a result of sea ice melting in spring/summer
...
CHL<-readRDS("D:/HeatWaves/Krill/Data/OISST_CHL.Rds")
...
ggplot(wsicL,aes(catsic,chlm))+geom_boxplot()+theme_bw()+
  xlab("Winter Sea Ice Cover")+ylab("Summer chlorophyll-a concentration (mg/m3)")
```
...
ggplot()+
  geom_smooth(data=wsicL,aes(x=sst,y=sic),method="gam",formula=y~s(x,k=3))+
  geom_point(data=wsicL,aes(x=sst,y=sic,colour=as.factor(Month)))+theme_bw()+
  ylab("Winter Sea Ice Cover")+xlab("Winter Sea Surface Temperature (°C)")

bartlett.test(wsicL$sic,wsicL$Lag)
shapiro.test(wsicL$sic)

lmp00<-lmPerm::lmp(sic~sst,data=wsicL,perm="Prob",center=T,singular.ok = F,maxIter=2000)
...
lmp5<-lmPerm::lmp(chlm~catsic,data=wsicL,perm="Prob",center=T,singular.ok = F)
...
lmp6<-lmPerm::lmp(chlm~cattemp,data=sumtL,perm="Prob",center=T,singular.ok = F)
